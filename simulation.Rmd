...
1. How does the sample mean compare to the theoretical mean of the distribution $\mu$.
2. How does the sample variance compare to the theoretical variance of the distribution $\sigma$.
3. How does the distribution compares to a normal distribution $N(\mu,\sigma^2/n)$.
...
In order to illustrate this, the distribution is plotted below (density plot). 
...
**Figure 2: distribution of averages of 40 exponential random variables with rate 
...
In order to compare the distribution of sample means to the Central Limit Theorem (CLT),
we plot below the distribution (histogram) along with a normal distribution $N(\mu,\sigma/\sqrt{n})$.
...
**Figure 3: distribution of averages of 40 exponential random variables (rate 
$\lambda = 0.2$). The mean and standard deviation of this distribution are respectively
...
As an alternative to compare the two distributions one can also draw a quantile-quantile plot,
 this is shown in figure A3 of the appendix.
...
```{r, cache = TRUE, fig.width=7, fig.height=3}
library(gridExtra)
set.seed(1332)
nosim  <- 1000
lambda <- 0.2  # rate parameter
